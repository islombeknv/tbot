...
async def Admincall(call: CallbackQuery):
    ...
    data = requests.get(f'http://127.0.0.1:8000/order/{pk}').json()
...
async def Adminback(message: Message, state: FSMContext):
    ...
    date = requests.get(f'http://127.0.0.1:8000/order/{data.get("pk")}').json()['date']
...
async def AdminAddDate(message: Message, state: FSMContext):
    ...
    kuryermessage = requests.get(f'http://127.0.0.1:8000/order/{pk}').json()['kuryer']
...
async def AdminKuryerClear(call: CallbackQuery):
    ...
    date = requests.get(f'http://127.0.0.1:8000/order/{pk}').json()['date']
...
async def AdminKuryerClear(call: CallbackQuery):
    ...
    kuryer = requests.get(f'http://127.0.0.1:8000/order/{pk}').json()['kuryer']
